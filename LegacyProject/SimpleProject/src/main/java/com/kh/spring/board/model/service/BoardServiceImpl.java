...
public class BoardServiceImpl implements BoardService {
	...
	private void validateContent(BoardDTO board) {
		if(board.getBoardTitle().trim().isEmpty() ||
				...
				String boardContent = board.getBoardContent().replace("<", "&lt");
				
				if(board.getBoardTitle().contains("강현성바보")) {
					boardTitle = board.getBoardTitle().replace("강현성바보", "글쓴사람바보");
				}
